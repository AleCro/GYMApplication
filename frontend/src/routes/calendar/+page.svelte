<script>
  ...
  // Ensure data shape before any mutation
  function ensureCalendar() {
    if (!data) data = { user: { session: '', calendar: [] } };
    if (!data.user) data.user = { session: '', calendar: [] };
    if (!Array.isArray(data.user.calendar)) data.user.calendar = [];
  }
  ...
  async function submitForm(e) {
    ...
    try {
      ...
      // Ensure structure and mutate safely
      ensureCalendar();
      const next = [
        ...data.user.calendar,
        {
      ...
      data = { ...data, user: { ...data.user, calendar: next } };
  ...
  async function deleteEvent(index) {
    ...
    try {
      ...
      ensureCalendar();
      if (index >= 0 && index < data.user.calendar.length) {
        const next = [...data.user.calendar];
        next.splice(index, 1);
        data = { ...data, user: { ...data.user, calendar: next } };
      }
