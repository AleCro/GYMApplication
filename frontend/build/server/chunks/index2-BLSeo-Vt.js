import { clsx } from "./clsx-cC83_lR5.js";
...
var Batch = class Batch {
	...
	*/
	process(root_effects) {
		...
		for (const root of root_effects) this.#traverse_effect_tree(root);
		if (this.#pending === 0) {
			this.#commit();
		...
		revert();
		for (const effect of this.#boundary_async_effects) update_effect(effect);
	...
	#traverse_effect_tree(root) {
		...
		while (effect !== null) {
			...
			if (!skip && effect.fn !== null) {
				...
				else if ((flags & EFFECT) !== 0) this.#effects.push(effect);
				else if ((flags & CLEAN) === 0) {
					if ((flags & ASYNC) !== 0 && effect.b?.is_pending()) this.#boundary_async_effects.push(effect);
	...
	*/
	...
	flush() {
		...
		} else if (this.#pending === 0) this.#commit();
		this.deactivate();
	}
	/**
		...
		if (batches.size > 1) {
			...
			for (const batch of batches) {
				...
				for (const [source2, value] of this.current) {
					if (batch.current.has(source2)) if (is_earlier) batch.current.set(source2, value);
					else continue;
	...
	while (i < length) {
		...
		if ((effect.f & (DESTROYED | INERT)) === 0 && is_dirty(effect)) {
			...
			if (eager_block_effects?.length > 0) {
				old_values.clear();
				for (const e of eager_block_effects) update_effect(e);
				eager_block_effects = [];
			}
		}
	}
...
function set(source2, value, should_proxy = false) {
	if (active_reaction !== null && (!untracking || (active_reaction.f & INSPECT_EFFECT) !== 0) && is_runes() && (active_reaction.f & (DERIVED | BLOCK_EFFECT | ASYNC | INSPECT_EFFECT)) !== 0 && !current_sources?.includes(source2)) state_unsafe_mutation();
...
function mark_reactions(signal, status) {
	...
	for (var i = 0; i < length; i++) {
		...
		if ((flags & DERIVED) !== 0) mark_reactions(
			/** @type {Derived} */
			reaction,
			MAYBE_DIRTY
		);
			...
			if ((flags & BLOCK_EFFECT) !== 0) {
				if (eager_block_effects !== null) eager_block_effects.push(
					/** @type {Effect} */
					reaction
				);
			}
...
function create_effect(type, fn, sync, push2 = true) {
	...
	if (push2) {
		var e = effect;
		if (sync && e.deps === null && e.teardown === null && e.nodes_start === null && e.first === e.last && (e.f & EFFECT_PRESERVED) === 0) e = e.first;
		if (e !== null) {
			e.parent = parent;
...
//#endregion
export { BOUNDARY_EFFECT, Batch, COMMENT_NODE, DEV, EFFECT_PRESERVED, EFFECT_TRANSPARENT, HYDRATION_END, HYDRATION_ERROR, HYDRATION_START, HYDRATION_START_ELSE, LEGACY_PROPS, active_effect, active_reaction, array_from, attr, attr_class, bind_props, block, branch, clear_text_content, component_context, component_root, create_text, define_property, destroy_effect, effect_pending_updates, effect_tracking, ensure_array_like, escape_html, experimental_async_ssr, flushSync, get, getContext, get_first_child, get_next_sibling, handle_error, head, hydration_failed, increment, init_operations, internal_set, invoke_error_boundary, is_array, is_passive_event, mutable_source, noop, pause_effect, pop$1, push$1, queue_micro_task, render, render_effect, safe_not_equal, set, setContext, set_active_effect, set_active_reaction, set_component_context, slot, source, ssr_context, store_get, stringify, svelte_boundary_reset_onerror, unsubscribe_stores, untrack };
//# sourceMappingURL=index2-BLSeo-Vt.js.map